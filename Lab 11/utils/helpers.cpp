...
void print_vector(int buffer[], size_t vector_length) {
    printf("[");
    if (vector_length == (size_t)-1) {
        for (size_t index = 0; buffer[index] != -1; index++)
...
int choice(const std::vector<int>& choices) {
    srand(time(NULL));
    size_t picked = rand() % choices.size();
...
void show_results(const float exercise,
  ...
  double expected = 0.0;
  double achieved = 0.0;
  size_t elem_count = std::min(expectations.size(), achievements.size());
